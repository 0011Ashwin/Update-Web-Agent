...
async def _handle_new_step(
    ...
    # Get Eleven Labs settings from webui_manager
    eleven_labs_api_key = _get_config_value(webui_manager, webui_manager.get_components_dict(), "agent_settings.eleven_labs_api_key", default=None)
    eleven_labs_voice_id = _get_config_value(webui_manager, webui_manager.get_components_dict(), "agent_settings.eleven_labs_voice_id", default=None)
    eleven_labs_language = _get_config_value(webui_manager, webui_manager.get_components_dict(), "agent_settings.eleven_labs_language", default="English")

    # Convert language string to 'en' or 'hi' for the utility function
    lang_code = "hi" if eleven_labs_language == "Hindi" else "en"

    # Example: Generate a conversational response and play it
    if eleven_labs_api_key:
        response_text = "Hello! How can I assist you today?"
        if lang_code == "hi":
            response_text = "नमस्ते! मैं आज आपकी कैसे मदद कर सकता हूँ?"
        try:
            from src.utils.utils import text_to_speech_eleven_labs
            text_to_speech_eleven_labs(response_text, eleven_labs_api_key, eleven_labs_voice_id, lang_code)
        except Exception as e:
            print(f"Error playing conversational response: {e}")
    
    # Add conversational response capability
    if eleven_labs_api_key and eleven_labs_voice_id:
        from src.utils.utils import text_to_speech_eleven_labs
        try:
            # Generate conversational response in English or Hindi based on user input
            response_text = f"I've completed step {step_num}. Here are the details: {formatted_output}"
            text_to_speech_eleven_labs(
                text=response_text,
            ...
            logger.error(f"Error in Eleven Labs text-to-speech: {e}")

    # Convert agent output to speech if Eleven Labs API key is provided
    if eleven_labs_api_key:
        from src.utils.utils import text_to_speech_eleven_labs
        text_to_speech_eleven_labs(output.parsed_output, api_key=eleven_labs_api_key, voice_id=eleven_labs_voice_id)

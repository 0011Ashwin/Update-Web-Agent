...
class WebuiManager:
    ...
    def get_components_dict(self) -> dict[str, "Component"]:
        """
        Get all components as a dictionary mapping component ID to component object
        """
    ...
    def save_config(self, components: Dict["Component", str]) -> None:
        ...
        for comp in components:
            if not isinstance(comp, gr.Button) and not isinstance(comp, gr.File) and str(
                ...
                # Handle password fields separately to avoid saving sensitive info directly if not intended
                if "api_key" in comp_id or "password" in comp_id:
                    # Only save if the user explicitly provided a value, otherwise keep it from .env
                    if components[comp]:
                        cur_settings[comp_id] = components[comp]
                    else:
                        # If empty, don't save, let it default to .env or existing value
                        pass
                else:
                    cur_settings[comp_id] = components[comp]
        ...
        for comp_id, comp_val in ui_settings.items():
            if comp_id in self.id_to_component:
                ...
                if comp.__class__.__name__ == "Chatbot":
                    ...
                    # For password fields, only update if a value is present in the loaded config
                    if "api_key" in comp_id or "password" in comp_id:
                        if comp_val:
